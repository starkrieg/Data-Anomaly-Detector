 ...
 * Follows a Naive algorithm, meant to validate calculation result during tests
 */
public class ZScoreAnomalyDetectorNaive implements DataAnomalyDetector {
    ...
    public ZScoreAnomalyDetectorNaive(int queueWindowSize) {
    ...
    private void checkIfDataAnomaly(double dataPoint) {
        ...
        // Since this is the Naive implementation meant for the Test structure
        // No need to perform logging here

        if (this.standardDeviation > 0) {
            // Update last Z-Score calculated
            this.lastZScore = Math.abs(dataPoint - this.mean) / this.standardDeviation;
        }
    }
